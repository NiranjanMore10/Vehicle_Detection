...
class Pipeline(object):
    ...
    def __init__(self, args, cfg,is_video=True):
...
def get_static_args():
    ...
    args.opt = {}
    args.config = 'deploy/pipeline/config/infer_cfg_ppvehicle.yml'
    args.image_file = 'test_image.jpeg'
    args.image_dir = None
    ...
    args.cpu_threads = 1
    args.trt_min_shape = 1
    args.trt_max_shape = 1280
    args.trt_opt_shape = 640
    args.trt_calib_mode = False
    args.do_entrance_counting = False
    args.do_break_in_counting = False
    args.illegal_parking_time = -1
    args.region_type = 'horizontal'
    args.region_polygon = []
    args.secs_interval = 2
    args.draw_center_traj = False
...
def main():
    FLAGS = get_static_args()
    FLAGS.device = 'GPU'
    assert FLAGS.device in ['CPU', 'GPU', 'XPU', 'NPU', 'GCU'], "device should be CPU, GPU, XPU, NPU or GCU"

    cfg = merge_cfg(FLAGS)
    print_arguments(cfg)
    get_model_dir(cfg)
    cfg["MOT"]["model_dir"] = "./output_inference/mot_ppyoloe_l_36e_ppvehicle"
    print("Using MOT model from:", cfg["MOT"]["model_dir"])
    image_files, video_files = get_input_files(FLAGS.input_dir)
    if image_files:
        print(f"Processing {len(image_files)} image files...")
        for img_path in image_files:
            FLAGS.image_file = img_path
            FLAGS.video_file = None
            pipeline = Pipeline(FLAGS, cfg,is_video=False)
            pipeline.run_multithreads()
    if video_files:
        print(f"Processing {len(video_files)} video files...")
        for vid_path in video_files:
            FLAGS.video_file = vid_path
            FLAGS.image_file = None
            pipeline = Pipeline(FLAGS, cfg,is_video=True)
            pipeline.run_multithreads()
