...
def load_predictor(model_dir,
    ...
    if paddle.__version__ >= '3.0.0' or paddle.__version__ == '0.0.0':
        model_prefix = 'model'
        param_path = os.path.join(model_dir, f'{model_prefix}.pdiparams')
        model_path = os.path.join(model_dir, f'{model_prefix}.pdmodel')
        ...
        if not os.path.exists(param_path) or not os.path.exists(model_path):
            ...
            if paddle.framework.use_pir_api():
                ...
                model_path = os.path.join(model_dir, f'{model_prefix}.json')

            if not os.path.exists(model_path) or not os.path.exists(param_path):
                raise ValueError(f"Cannot find any inference model in dir: {model_dir}.")
